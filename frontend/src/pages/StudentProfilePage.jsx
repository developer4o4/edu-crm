...
export default function StudentProfilePage() {
  const { user, logout, myStudentProfile } = useAuthStore()
  ...
  useEffect(() => {
    ...
    if (tab === 'payments') {
      studentsAPI.payments(student.id).then(r => setPayments(r.data)).catch(() => setPayments({ payments: [], total_paid: 0 }))
    }
    if (tab === 'attendance') {
      studentsAPI.attendance(student.id).then(r => setAttendance(r.data)).catch(() => setAttendance(null))
    }
  }, [tab, student])
                  ...
                  <div key={m.id} style={{ border: '1px solid #e1f5ee', borderRadius: '12px', padding: '16px', background: '#f0fdf8' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px' }}>
                      <div style={{ background: 'white', borderRadius: '8px', padding: '10px' }}>
                        ...
                        {m.teacher_phone && (
                          <div style={{ marginTop: '6px', fontSize: '12px', color: '#6b7280' }}>{m.teacher_phone}</div>
                        )}
                      </div>
                      <div style={{ background: 'white', borderRadius: '8px', padding: '10px' }}>
                      ...
                      <div style={{ background: 'white', borderRadius: '8px', padding: '10px' }}>
                        <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '2px' }}>Darslar</div>
                        <div style={{ fontWeight: '500', color: '#111827' }}>{m.lessons_held || 0} ta</div>
                      </div>
                      <div style={{ background: 'white', borderRadius: '8px', padding: '10px' }}>
                        <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '2px' }}>Dars vaqti</div>
                        <div style={{ fontWeight: '500', color: '#111827' }}>{m.schedule?.day_type} {m.schedule?.start_time}-{m.schedule?.end_time}</div>
                      </div>
                    </div>
