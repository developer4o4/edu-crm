...
import { Plus, Search, Eye, Trash2, Phone, Filter, X } from 'lucide-react'
...
export default function StudentsPage() {
  ...
  const clearAllFilters = () => {
    ...
    setDateFrom(''); setDateTo(''); setDebtors(false)
    setPage(1)
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Btn primary onClick={() => setModal(true)}>
            <Plus size={14} /> Yangi o'quvchi
          </Btn>
        }
      />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#fafafa' }}>
                  {["O'quvchi", "Telefon", "Guruhlar", "Status", "Sana", ""].map(h => (
                    <Th key={h}>{h}</Th>
                  ))}
                </tr>
              </thead>
            ...
            {meta.total_pages > 1 && (
              <div style={{ padding: '12px 16px', borderTop: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#9ca3af' }}>
                  Sahifa {page}/{meta.total_pages} ({meta.count} ta)
                </span>
