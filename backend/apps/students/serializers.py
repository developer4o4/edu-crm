...
class MembershipSerializer(serializers.ModelSerializer):
    ...
    lessons_held = serializers.SerializerMethodField()

    class Meta:
        model = GroupMembership
        fields = ['id', 'group', 'group_name', 'course_name', 'teacher_name', 'teacher_phone',
                  'monthly_fee', 'monthly_fee_discounted', 'discount_percent',
                  'joined_at', 'is_active', 'schedule', 'lessons_held']
    ...
    def get_teacher_phone(self, obj):
        if obj.group.teacher:
            return obj.group.teacher.phone
        return None

    def get_lessons_held(self, obj):
        from apps.attendance.models import Attendance
        return Attendance.objects.filter(
            student=obj.student,
            session__group=obj.group,
        ).count()
